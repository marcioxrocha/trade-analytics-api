...
export class AppController {
  ...
  async negociosUpdate(@Req() request, @UploadedFile() file: Express.Multer.File) {
    ...
    if(file.originalname != 'negocios.csv'){
      return;
    }    
  ...
  @Post('backtest/update')
  @UseInterceptors(FileInterceptor('backtest'))
  async backtestUpdate(@Req() request, @UploadedFile() file: Express.Multer.File) {
    let url = `${request.headers.url}/rest/v1/backtest?on_conflict=id`;
    let token = request.headers.token;
    ...
    if(file.originalname != 'backtest.csv'){
      return;
    }
    ...
    await lastValueFrom(
      this._httpService.delete(`${request.headers.url}/rest/v1/backtest?id=not.is.null`, {
        headers: {
          'apikey': token,
          'Authorization': `Bearer ${token}`,
        }
